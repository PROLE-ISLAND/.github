...
def handle_issue_create(command: str, args: List[str], guardrails: Dict[str, Any]) -> None:
    ...
    if body_file and not is_routed:
        recommended = get_recommended_command(args)
        warnings.append(f"⚠️ コマンド経由での実行を推奨: {recommended}")
        warnings.append(f"   └─ 直接実行は品質チェックが不完全になる可能性があります")
...
def handle_pr_create(args: List[str], guardrails: Dict[str, Any]) -> None:
    ...
    if body_file and not is_routed:
        recommended = get_recommended_command(args)
        warnings.insert(0, f"⚠️ コマンド経由での実行を推奨: {recommended}")
        warnings.insert(1, f"   └─ 直接実行は品質チェックが不完全になる可能性があります")
